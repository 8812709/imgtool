...
@app.post("/resize")
async def resize_image(name:str,roll_no:int,passportphoto:UploadFile=File(),width=300,height=300):
    ...
    try:
        ...
        raise HTTPException(status_code=401,details="some error occured while opening the file:{str(e)}")
    try:
        resized_image = img.resize((width,height)) # Resize the image to 300*300 ratio using the syntax "resize"
    except Exception as e:
        raise HTTPException(status_code=402,detail="some kind of error while resizing the image:{str(e)}")
    try:
        resized_image.save("newimage1.png")  #saves the image in my pc in imgtool folder
    except Exception as e:
        raise HTTPException(status_code=403,detail="some kind of error occured while saving the file in the directory,may possible that the directory dosnt exist")
